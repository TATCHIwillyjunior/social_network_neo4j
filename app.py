...
# ======================
class Database:
    def __init__(self, uri="neo4j+s://36283c89.databases.neo4j.io", user="neo4j"):
        self.driver = GraphDatabase.driver(uri, auth=(user, "neo4j"))
        self._init_db()
    ...
    def create_user(self, username: str, name: str) -> int:
        query = """
            ...
            """
        result = self.run(query, {"username": username, "name": name})
    ...
    def get_user(self, user_id: int) -> Optional[dict]:
        query = """
            ...
            """
        result = self.run(query, {"user_id": user_id}).single()
    ...
    def create_post(self, user_id: int, content: str) -> int:
        query = """
            ...
            RETURN p.id AS id
            """
        result = self.run(query, {"user_id": user_id, "content": content})
        return result.single()["id"]
    ...
    def get_posts_by_user(self, user_id: int) -> List[dict]:
        query = """
            ...
            """
        result = self.run(query, {"user_id": user_id})
        return [dict(record) for record in result]
    ...
    def follow_user(self, follower_id: int, followee_id: int) -> bool:
        ...
        """
        result = self.run(
            query, {"follower_id": follower_id, "followee_id": followee_id}
        )
    ...
    def unfollow_user(self, follower_id: int, followee_id: int) -> bool:
        query = """
                MATCH (follower:User {id: $follower_id})-[rel:FOLLOWS]->(followee:User {id: $followee_id})
                DELETE rel
                RETURN true AS success
            """
        result = self.run(
